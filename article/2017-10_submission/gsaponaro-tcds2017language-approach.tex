...
\begin{figure*}
  \tikzstyle{affnode} = [ellipse, draw, fill=gray!20, thick]%node distance=1cm, text width=6em, text centered, minimum height=4em, thick]
  \tikzstyle{wordnode} = [ellipse, draw, thick]
  \tikzstyle{group} = [rectangle, draw, black, thick]%, inner sep=0.5cm
  \tikzstyle{dashedgroup} = [rectangle, draw, inner sep=0.5cm, dashed, rounded corners, black, thick]
  ...
  \begin{minipage}{0.5\textwidth}
    ...
    \begin{tikzpicture}
      ...
      \draw[->, thick] ([xshift=30pt]effects.south) -- ([xshift=30pt]words.north);
      % extra
      \node[affnode, above of=actions] (gestures) [above=0.7cm] {Gesture Features};
