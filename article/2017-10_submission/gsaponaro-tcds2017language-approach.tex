...
\begin{figure*}
  ...
  \centering
  \subfloat[][ideal case: the affordance and word variables are dependent on the time-evolving detailed description of the action.]{
    \centering
    \begin{tikzpicture}
      % single nodes
      \node[affnode] (g1) {$g_1$};
...
To simplify notation, we call $X = \{A, F, E, W\}$ the set of affordance and word variables $\{a, f_1, f_2, \dots, e_1, e_2, \dots, w_1, w_2, \dots\}$.
...
\end{enumerate}
We are here excluding the case that we observe the action directly ($A\in \xobs$) for two reasons: firstly, this would correspond to the robot perfroming the action itself, whereas we are interested in interpreting other people's actions;
...
The means that we can evaluate the two models independently and multiply the distribution we obtain from the Bayesian network over all the possible value of the inference variables, by the HMM posterior for the corresponding value of the action.
