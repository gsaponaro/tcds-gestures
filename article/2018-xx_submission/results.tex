...
In the scene, we observe a small ball which, after the manipulative action, shows a low velocity.
...
Recall that with our method we do not add new words to the model, when we observe the human perform actions.
Rather, the human-readable descriptions we generate, are based on the same words that were present in the self-centered learning phase.
In this phase, the verbal descriptions described the agent of the observed actions as either ``the~robot'', ``he'', or ``Baltazar''~(the name of the robot).
