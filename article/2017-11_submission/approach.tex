...
There are many synonyms for the same concept. Cubic object are \textit{e.g.} called ``box'', ``square'' or ``cube''.
Different node values may have the same associated verbal description.
There are two color clusters, \textit{e.g.}, that correpond to differnt shades of green that are always referred to as ``green''.
Finally, many affordance node values do not have any direct description.
For example, the object velocity and object-hand velocities (slow, medium, fast), or the object-hand contact (short, long) do not have any direct description, but are inferred from the situation.
