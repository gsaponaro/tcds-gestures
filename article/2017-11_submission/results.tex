...
\begin{table}
    \centering
    \caption{Sentences generated from the evidence $\xobs = \{\text{Color=yellow, Size=big, Shape=circle, ObjVel=fast}\}$ and ranked according to~\eqref{eq:sentence_score}. The higher the score, the better.}
...
\newcommand{\evidenceProducingAnd}{'Size', 'small', 'Shape', 'circle', 'Action', 'grasp'}
...
\begin{table*}
    \centering
    \caption{Sentences generated from the evidence \emph{\{\evidenceProducingAnd\}}. The ``and'' conjunction was chosen~8/10 times. See also Fig.~\ref{fig:conjunction_and:pw}.}
    \label{tab:conjunction:and}
    \begin{tabular}{ll}
    \toprule
    sentence & score \\
    \midrule
    baltazar is picking the sphere \textbf{and} the ball is rising & $-0.59377$ \\
    he picked the sphere \textbf{and} the ball moves & $-0.6413$ \\
    baltazar grasped the ball \textbf{and} the ball rises & $-0.64737$ \\
    baltazar grasps the ball \textbf{and} the green ball is moving & $-0.64832$ \\
    the robot just picked the small sphere \textbf{and} the sphere is rising & $-0.6809$ \\
    the robot is picking the blue sphere \textbf{and} the sphere is rising & $-0.73636$ \\
    the robot picks the green ball \textbf{and} the green ball is still & $-0.76653$ \\
    the robot just picked the ball \textbf{but} the ball is rising & $-0.80258$ \\
    baltazar picks the green sphere \textbf{and} the sphere is still & $-0.80352$ \\
    the robot picks the ball \textbf{but} the sphere is inert & $-0.81117$ \\
    \bottomrule
    \end{tabular}
\end{table*}
...
\newcommand{\evidenceProducingBut}{'Size', 'small', 'Shape', 'box', 'Action', 'grasp'}

Example in which the generated conjunction is ``but'' (because of no consistency):

evidence: \evidenceProducingBut
...
\begin{table*}
    \centering
    \caption{Sentences generated from the evidence \emph{\{\evidenceProducingBut\}}. The ``but'' conjunction was chosen~3/10 times, most importantly in the highest-ranked sentence. See also Fig.~\ref{fig:conjunction_but:pw}.}
    \label{tab:conjunction:but}
    \begin{tabular}{ll}
    \toprule
    sentence & score \\
    \midrule
    the robot grasped the box \textbf{but} the square is inert & $-0.66595$ \\
    the robot grasped the cube \textbf{and} the square is inert & $-0.67108$ \\
    the robot picks the square \textbf{and} the square is inert & $-0.67156$ \\
    the robot picks the square \textbf{and} the square is rising & $-0.70709$ \\
    baltazar is picking the cube \textbf{but} the square is still & $-0.72525$ \\
    he is grasping the square \textbf{and} the square is rising & $-0.72734$ \\
    the robot grasped the small cube \textbf{and} the box is falling & $-0.73091$ \\
    he is picking the green square \textbf{and} the cube is inert & $-0.74436$ \\
    baltazar grasped the box \textbf{and} the box is inert & $-0.74621$ \\
    he is grasping the box \textbf{but} the box is rising & $-0.74632$ \\
    \bottomrule
    \end{tabular}
\end{table*}
