...
In Fig.~\ref{fig:impact_of_evidence_on_Action} we show the interaction between the affordance-word model and the gesture/action recognition model when they bring conflicting information.
In the scene we observe a small ball that after the action has a low velocity (that in terms clusters of affordance variables often corresponds to zero velocity).
...
However, in this paricular simulated situation, we assume that the action performed by the human was an (unsuccesfull) \emph{tap}, that is a tap that does not result in any movement for the object.
...
In the simulation we show the inference based on $\pcomb(A\mid \xobs,G_1^N)$ wehn we vary the degree of confidence of the recognizer.
%compactly the result of eight different queries~(each group of bars), corresponding to the same number of eight different soft evidences over the action, ranging from a non-informative, uniform distribution~(i.e.,~[1/3~1/3~1/3]), up to the maximum score being attributed to the ``tap'' action~(i.e.,~[0~1~0]), with intermediate distributions in between.
...
For instance, by providing the following evidence to the model:
