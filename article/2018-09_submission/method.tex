...
In this phase human movements are interpreted using the experience acquired so far, and incorporated into the model using a statistical gesture recognizer similar to~\cite{saponaro:2013:crhri}.
...
In our implementation, we hinge on the existence of the discrete variable Action, the value of which is known to the robot in the ego-centric phase of learning, but must be inferred when observing humans actions.
This variable links all the other observable variables in the model, that is, human gesture features, object properties, effect variables and words.
